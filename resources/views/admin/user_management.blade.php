    ...
    <div class="card mb-4" style="box-shadow: 12px 12px 7px rgba(0, 0, 0, 0.3);">
        ...
        <div class="card-body">
            <table id="datatablesSimple" class="table table-bordered table-striped">
                ...
                <tbody>
                @forelse($users->where('user_role', '!=', 'admin') as $user)
                        <tr>
                            <td>{{ $user->emp_id }}</td>
...
<div class="modal fade" id="addUserModal" tabindex="-1" aria-labelledby="addUserModalLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="addUserForm" method="POST" action="{{ route('admin.add_user') }}">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required>
                            ...
                            <option value="salesclerk" {{ old('role') == 'SalesClerk' ? 'selected' : '' }}>Sales Clerk</option>
                        </select>
                        @error('role')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        editUserModal.addEventListener('show.bs.modal', function (event) {
            ...
            form.action = '{{ route('admin.edit_user', ['id' => '__id__']) }}'.replace('__id__', userId);
        })
    });
...
changePasswordModal.addEventListener('show.bs.modal', function (event) {
    ...
    form.action = '{{ route('admin.change_password', ['id' => '__id__']) }}'.replace('__id__', userId);
})
</script>
